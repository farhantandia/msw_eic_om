        ...
        function getSubtaskTypeBadge(desc) {
            ...
            // 1. Actuator Check
            const isAct = (fullData.master_actuators || fullData.actuators || []).some(a => {
                const kks = (a.kks || '').toUpperCase();
                const eq = (a.equipment_description || '').toUpperCase();
                if(kks && kks.length >= 6 && s.includes(kks)) return true;
                if(eq && eq.length >= 8 && (s.includes(eq) || eq.includes(s))) return true;
                return false;
            }) || s.includes('ACTUATOR') || s.includes(' MOV ') || s.endsWith(' MOV') || s.includes('GATE ACTUATOR') || s.includes('DAMPER ACTUATOR') || s.includes('VALVE ACTUATOR') || s.includes('AOV');
            
            if(isAct) {
                return '<span class="badge-tag-comp badge-tag-act">ACTUATOR</span>';
            }
            
            // 2. Instrument Check (PTX, TTX, PSW)
            const allInst = [...(fullData.pressure_tx||[]), ...(fullData.temperature_tx||[]), ...(fullData.pressure_switch||[]), ...(fullData.master_instruments||[])];
            const isInst = allInst.some(inst => {
                const kks = (inst.kks || '').toUpperCase();
                const eq = (inst.equipment || inst.description || '').toUpperCase();
                if(kks && kks.length >= 6 && s.includes(kks)) return true;
                if(eq && eq.length >= 10 && (s.includes(eq) || eq.includes(s))) return true;
                return false;
            }) || s.includes('TRANSMITTER') || s.includes('PRESSURE SWITCH') || s.includes('TEMPERATURE SWITCH') || s.includes('CALIBRATION MEASUREMENT') || s.includes('KALIBRASI TRANSMITTER');
            
            if(isInst) {
                return '<span class="badge-tag-comp badge-tag-inst">INSTRUMENT</span>';
            }
            
            // Jika tidak ada di list actuator dan instrument -> jangan tampilkan label apapun
            return '';
        }
                if(aDesc && aDesc.length >= 10 && (s.includes(aDesc) || aDesc.includes(s))) {
                    return '<span class="badge-tag-comp badge-tag-act" title="Tersinkron dengan Tab Actuator">ACTUATOR</span>';
                }
            }
            
            // 2. Cek kecocokan dengan Master Instrument List (PTX, TTX, PSW)
            const insts = [...(fullData.pressure_tx || []), ...(fullData.temperature_tx || []), ...(fullData.pressure_switch || [])];
            for(let i = 0; i < insts.length; i++) {
                const inst = insts[i];
                const iKks = (inst.kks || '').trim().toUpperCase();
                const iDesc = (inst.equipment || inst.description || '').replace(/\s+/g, ' ').trim().toUpperCase();
                if(iKks && iKks.length >= 6 && s.includes(iKks)) {
                    return '<span class="badge-tag-comp badge-tag-inst" title="Tersinkron dengan Tab Instrument">INSTRUMENT</span>';
                }
                if(iDesc && iDesc.length >= 10 && (s.includes(iDesc) || iDesc.includes(s))) {
                    return '<span class="badge-tag-comp badge-tag-inst" title="Tersinkron dengan Tab Instrument">INSTRUMENT</span>';
                }
            }
