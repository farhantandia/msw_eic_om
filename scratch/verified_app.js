        ...
        function getSubtaskTypeBadge(desc) {
            ...
            for(let i = 0; i < acts.length; i++) {
                const a = acts[i];
                const aKks = (a.kks || '').trim().toUpperCase();
                const aDesc = (a.equipment_description || '').replace(/\s+/g, ' ').trim().toUpperCase();
                if(aKks && aKks.length >= 6 && s.includes(aKks)) {
            ...
            if(s.includes('ACTUATOR') || s.includes(' MOV ') || s.endsWith(' MOV') || s.includes('GATE ACTUATOR') || s.includes('DAMPER ACTUATOR') || s.includes('VALVE ACTUATOR') || s.includes('AOV')) {
                return '<span class="badge-tag-comp badge-tag-act">ACTUATOR</span>';
            }
            ...
            if(s.includes('TRANSMITTER') || s.includes('PRESSURE SWITCH') || s.includes('TEMPERATURE SWITCH') || s.includes('CALIBRATION MEASUREMENT') || s.includes('KALIBRASI TRANSMITTER')) {
                return '<span class="badge-tag-comp badge-tag-inst">INSTRUMENT</span>';
            }
            
            // Subtask umum / motor / lainnya -> TANPA LABEL
            return '';
        }
